...
class Post{
    ...
    private function createEditPost($postId){
        ...
        $short = $shortImages["content"];
        $content = $contentImages["content"];

        if(!empty($shortImages["images"]) || !empty($contentImages["images"])){
            $imagesContent = array_merge($shortImages["images"], $contentImages["images"]);
        }
        ...
        if(!$postId){ // если это добавление новой категории
            ...
            if(!empty($imagesContent)){
                foreach ($imagesContent as $image) {
                    $PostModel->addImage(1, $postId, $image);
                }
            }
    ...
    private function parseImages($content){
        ...
        if(!empty($images[2])){
            ...
            foreach ($images[2] as $key => $imageContent) {
                ...
                $dir_rel = date("Y-m", time());
                $filePath = ROOT . '/uploads/posts/' . $dir_rel;
                ...
                $imageResult = '<img src="//'.CONFIG_SYSTEM["home"].'/uploads/posts/'.$dir_rel.'/'.$fileName.'" alt="">';
                ...
                if(CONFIG_SYSTEM["thumb"] && CONFIG_SYSTEM["quill_thumbs"]){
                    ...
                    $imageResult = '<a href="//'.CONFIG_SYSTEM["home"].'/uploads/posts/'.$dir_rel.'/'.$fileName.'"><img src="//'.CONFIG_SYSTEM["home"].'/uploads/posts/'.$dir_rel.'/thumbs/'.$fileName.'" alt=""></a>';
                }
    ...
    private function deleteImage(){
        ...
        if(file_exists(ROOT . "/uploads/posts/".$link))
     ...
     * @return array|void
     */
    private function uploadContentImages($imagesSource){

        $images = [];

        foreach ($imagesSource as $image_name) {

            $ext = mb_strtolower(pathinfo($image_name, PATHINFO_EXTENSION), 'UTF-8'); // расширение файла
            ...
            if(
                ...
                // по идее директория уже создана
                $dir = ROOT . '/uploads/posts/'.date("Y-m", time()).'/';
