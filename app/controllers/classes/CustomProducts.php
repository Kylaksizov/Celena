...
class CustomProducts{
    ...
    public function custom($e, $categories, $template = 'customProducts', $limit = 10, $order = 'id', $sort = 'desc'){
        ...
        else $Products = $ProductModel->getCustomProducts($categories, $limit, $order, $sort);

        if($tpl->route["controller"] == 'category'){ // если категория
            ...
            // CATEGORY NAME
            $categoryName = !empty($CategoryStep[end($e->urls)]) ? $CategoryStep[end($e->urls)]["title"] : '';
            $tpl->setMain('{category-name}', $categoryName);
